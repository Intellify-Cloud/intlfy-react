...
function createBeam(width: number, height: number): Beam {
  ...
  return {
    ...
    // Burnt orange range: ~15 (red-orange) to ~35 (amber)
...
export function BeamsBackground({
  ...
  // Track dark mode (watches the `dark` class on <html>)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      // Keep burnt orange palette on reset too
      beam.hue = 15 + ((index * 20) / totalBeams);
    ...
    function drawBeam(ctx: CanvasRenderingContext2D, beam: Beam) {
      ...
      // In light mode, reduce opacity significantly so beams are subtle on white
      const modeMultiplier = dark ? 1 : 0.35;
      const saturation = dark ? 90 : 75;
  ...
  return (
    <div
      className={cn(
        ...
        // Dark: near-black; Light: clean white
    ...
    >
      ...
      {/* Subtle pulsing overlay — tinted orange in dark, near-transparent in light */}
      <motion.div
        className={cn(
          "absolute inset-0",
